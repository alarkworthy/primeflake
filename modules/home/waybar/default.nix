{config,pkgs,lib,...}:
with lib;
let cfg = config.pluto.desktop.waybar;
in
{
  options.pluto.desktop.waybar.enable = mkEnableOption "Enable Waybar" // {default = config.pluto.desktop.sway.enable;};
  config = mkIf cfg.enable {
	...
	mainBar = {
	  ...
	  #height = 30;
	  #spacing = 6;
	  modules-left = ["user" "sway/workspaces"];
	  modules-center = ["sway/window"];
	  modules-right = [ "tray" "pulseaudio" "network" "cpu" "memory" "temperature" "sway/langauge" "clock"];
