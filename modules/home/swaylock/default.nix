...
in
{
	options.pluto.desktop.swaylock.enable = lib.mkEnableOption "Enable Swaylock" // {default =false;};
	options.pluto.desktop.swaylock.idle.enable = lib.mkEnableOption "Enable Swayidle" // {default=false;};
	...
	config = lib.mkMerge [
		...
		(lib.mkIf cfg.idle.enable {
			services.swayidle = {
				enable = true;
				timeouts = [
					{timeout=cfg.idle.time.lock; command= "${pkgs.swaylock}/bin/swaylock -fF";}
					{timeout=cfg.idle.time.suspend; command = "${pkgs.systemd}/bin/systemctl suspend";}
				];
				events = [
